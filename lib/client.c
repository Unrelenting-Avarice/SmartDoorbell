...
void client_send_image(int sockfd, const Config *config) {
    log_info("file desc - %d", sockfd);
    int data_length = config -> payload_size;
    int total_sent = 0; // Lump sum
    int amnt_sent = 0; // Per packet
    uint8_t *data = config -> payload;

    int lenhwID = 9;
    char* comb_data = malloc(lenhwID + data_length);
    memcpy(comb_data, "006FDF9FB", lenhwID);
    memcpy(comb_data + lenhwID, data, data_length);

    data_length += lenhwID;

    while (total_sent < data_length){
        amnt_sent = send(sockfd, comb_data, data_length, 0);
        log_info("amnt_sent %d, total sent %d", amnt_sent, total_sent);

        if (amnt_sent < 0){
            log_info("Failed sending data");
            break;
        }
        total_sent += amnt_sent; // Get tot sent
